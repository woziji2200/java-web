<template>
    <div id="range" class="background h-screen w-screen">
        ...
        <div class="desktop">
            <TaskVue v-for="i in AppList?.filter((i: props.Task) => i.recommend)" :icon="i.icon" @click="OpenApp(i)" :type="i.type"
...
const closeTask = (pid: Number) => {
    ...
    TasksList.value.splice(TasksList.value.findIndex((i: props.Task) => i.pid == pid), 1)
    newWindowsDec()
}
...
const showTask = (pid: Number) => {
    (TaskRef.value.filter((i: HTMLElement) => (toRaw(i) as any).pid as any == pid)[0] as any).showWindow()
}
...
const TaskDragStart = (e: Event, pid: Number, index: number) => {
    ...
    setTimeout(() => {
        TasksList.value.filter((i: props.Task) => i.pid == pid)[0].class = 'moving'
    }, 0);
}
...
const TaskDragEnd = (e: Event, pid: Number, index: number) => {
    TasksList.value.filter((i: props.Task) => i.pid == pid)[0].class = ''
}
...
const OpenApp = (app: props.Task) => {
    if(app.single){
        const task = TasksList.value.filter((i: props.Task) => i.name == app.name)
        if(task.length > 0){
            showTask(task[0].pid)
            return
        }
...
watch(AppSearch, (newVal: String, oldVal: String) => {
    if (newVal == '') {
        AppListShow.value = AppList.value
    } else {
        AppListShow.value = AppList.value?.filter((i: props.Task) => { return i.title.toString().toLowerCase().indexOf(newVal.toLowerCase()) != -1 && i.type == 'task' })
    }
})
...
const ShowTaskMenu = (e: MouseEvent, pid: Number) => {
    ...
    ContextMenu.showContextMenu({
        ...
        items: [{
            ...
            onClick: () => {
                (TaskRef.value.filter((i: props.Task) => (toRaw(i) as any).pid as any == pid)[0] as any).maximize()
            }
        }, {
            label: '最小化',
            icon: 'fa fa-window-minimize',
            onClick: () => {
                (TaskRef.value.filter((i: props.Task) => (toRaw(i) as any).pid as any == pid)[0] as any).minimize()
            }
        }, {
            label: '关闭窗口',
            icon: 'fa fa-window-close',
            onClick: () => {
                (TaskRef.value.filter((i: props.Task) => (toRaw(i) as any).pid as any == pid)[0] as any).close()
            }
        }]
    })
}
...
const showDesktop = () => {
    TaskRef.value.forEach((i: props.Task) => {
        (i as any).minimize()
    })
}
...
watch(AppMenuShow, (newVal: boolean, oldVal: boolean) => {
